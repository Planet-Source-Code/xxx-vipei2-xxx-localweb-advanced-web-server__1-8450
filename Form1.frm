...
Begin VB.Form Form1 
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      Begin VB.Label Label2 
         BackStyle       =   0  'Transparent
         Caption         =   "Home Page Avaliable at:"
         Height          =   345
         Left            =   210
         TabIndex        =   2
         Top             =   1095
 ...
 If Left(clstring, 3) = "GET" Then
    ...
    Else
      ...
      For f = 1 To Len(HTMFile)
        ...
        End If
      Next
      'check for a '/' if it isn't there, then we dont
      'have any directories to parse
      f = InStr(1, HTMFile, "/")
      If f > 0 Then
        length = Len(HTMFile)
        dot = InStr(1, HTMFile, ".")
        If dot > 0 Then
          Do While Mid(HTMFile, dot, 1) <> "/"
            dot = dot - 1
          Loop
          For f = dot + 1 To length
            Pagename = Pagename & Mid(HTMFile, f, 1)
          Next
          For f = 1 To dot
            dirname = dirname & Mid(HTMFile, f, 1)
          Next f
          'now convert virtual to real
          VirtualFound = False
          ...
          If VirtualFound Then
            ...
            HTMFile = Pagename
          Else
           'put in code if virtdir not found
          End If
        Else
          'we have a dir, but need a default page
          If Right(HTMFile, 1) <> "/" Then
            HTMFile = HTMFile & "/"
            dirname = HTMFile
          End If
          'now convert virtual to real
